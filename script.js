...
async function callBackend(url, network) {
  ...
  if (!res.ok) {
    ...
    throw new Error(body.message || `Greška servera (${res.status})`);
  }
...
function renderResult(data, url, network) {
  ...
  resultBody.textContent     = postText;
  resultHashtags.textContent = hashtagStr;
 
  /* Character count */
  const fullText  = [data.title, postText, hashtagStr].filter(Boolean).join('\n\n');
  ...
  resultMeta.innerHTML = `
    <span>Generisano ${new Date().toLocaleTimeString('sr-Latn-RS', { hour: '2-digit', minute: '2-digit' })}</span>
